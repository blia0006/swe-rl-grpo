...
padded_attention_mask = torch.cat([torch.zeros((1, pad_len), dtype=attention_mask.dtype), attention_mask], dim=1).cuda()
position_ids = (padded_attention_mask.cumsum(dim=-1) - 1).clamp(min=0).cuda()
print("real_len", real_len, "padded_len", padded_input_ids.shape[1], flush=True)
...
for seed in [0, 1, 2, 3, 4]:
    ...
    for i in range(out.shape[0]):
        ...
        print(f"=== seed={seed} sample={i} resp_len={len(resp)} ===", flush=True)
