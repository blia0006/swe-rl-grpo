...
#### 7.4.3 统计检验：表面的"下降"在统计上不成立

`pass@1 (partial)` 从 0.25 掉到 0，看起来是退化，但**必须先做显著性检验再下结论**：

**（1）平均 reward 差异（Welch t 检验，双样本 n=32）**

$$\bar{x}_{\text{before}}=0.0875\ (s=0.1264),\quad \bar{x}_{\text{after}}=0.0813\ (s=0.0998)$$

$$t = \frac{0.0813-0.0875}{\sqrt{s_1^2/32+s_2^2/32}} = \frac{-0.0062}{0.0285} = \mathbf{-0.22}$$

$|t| = 0.22 \ll 2$ → **差异完全不显著**，标准误 `0.0285` 是差值 `0.0062` 的 4.6 倍，
这个"下降"完全落在噪声范围内。

**（2）apply 成功率差异（两比例 z 检验）**

$$z = \frac{0.406-0.375}{\sqrt{\hat p(1-\hat p)\cdot 2/32}} = \mathbf{0.26}$$

同样不显著。**但方向是上升的**（37.5% → 40.6%），与训练日志中 `strict` 成功次数
从第 12 步的 2 次增至 55 步 8 次的趋势方向一致。

**（3）`pass@1 (partial)` 从 0.25 → 0 的真相**

这个指标看着跌了 25 个百分点，实际上它**由单个样本决定**：

- `before`：32 个采样里只有 **2 个**拿到非零测试分（都在 `swe-synth-0022`，reward 0.3 / 0.5）
- `after`：**0 个**

由于 `pass@1 (partial)` 是题级指标（4 题），**1 道题的得失就等于 25 个百分点**。
2/32 → 0/32 的变化在 32 个样本量下毫无统计意义（Fisher 精确检验 p ≈ 0.49），
**把它解读为"训练导致模型变差"是站不住的**。

**（4）结论**

> 在 4 题 × 8 采样的评测规模下，训练前后的差异**在统计上无法区分于噪声**。
> 唯一方向明确的信号是 apply 成功率小幅上升（+3.1pp，不显著）。
> 诚实的表述是：**本次 55 步 LoRA 训练未能产生可被当前评测规模检出的能力提升**，
> 而非"训练使模型退化"。

要得到有统计功效的结论，需要把评测规模提升到至少 20 题 × 16 采样（320 样本），
或直接提高训练规模（见 §7.5 改进方向）。

#### 7.4.4 一个新暴露的失败模式：文件路径幻觉

高灵敏度组的 `reason` 日志暴露了严格组没显现的问题：
...
即模型不仅算错行号，**还会写错文件路径**（漏前缀、加不存在的前缀）。这类样本连
`--recount -C0` 都救不回来（文件都找不到）。这是 §7.2 三层瓶颈之外的第四类失败，
**改进方向**：在 prompt 里更醒目地固定路径（如显式给出 `--- a/<完整路径>` 模板行），
或在 reward 中对"路径不存在"单独给出更低的档位以形成区分信号。
...
`_score_via_sandbox()` 以取回 `reason` 字段（`corrupt patch` / `does not apply` /
`No such file or directory` 等），写入 `records[].reason`，这样即使结果全 0 也能定位到
具体是哪一层失败 —— §7.4.4 那个路径幻觉问题正是靠它发现的。

### 7.5 改进方向（按预期收益排序）

按本次实测数据，最该做的不是换更大的模型，而是先修 reward 设计和任务表示：

**1. reward 分档细化（最高优先级，改动最小）**

当前 `0.2` 这一档同时包含了"patch 位置正确、测试可正常收集"和"靠 `-C0` 硬插、
文件语法已坏"两类样本（后者占 **51%**，见 §7.2），等于告诉模型"写歪的 diff 和写对的
diff 一样值钱"。建议改为：

| reward | 条件 |
|---|---|
| `0.00` | 未抽到 patch / 路径不存在 / 完全无法 apply |
| `0.05` | apply 成功但 `collect_error`（代码被插坏） |
| `0.20` | apply 成功且测试可正常收集 |
| `0.20 + 0.8×通过率` | 部分/全部修对 |

"写歪"与"写对"产生 4 倍差距，组内 advantage 才能推动格式能力提升。

**2. 缓解路径幻觉（§7.4.4）**

在 prompt 里把 `--- a/<完整路径>` / `+++ b/<完整路径>` 作为**固定模板行**直接给出，
让模型只需填 hunk 内容；或对"路径不存在"单独设更低档位形成区分信号。

**3. 提高评测统计功效**

当前 4 题 × 8 采样（32 样本）下，标准误是观测差值的 4.6 倍，任何结论都无法与噪声区分。
建议至少 20 题 × 16 采样（320 样本）。

**4. 扩大训练规模**

55 step / 11 题 / LoRA rank 16 对"学会精确计算 diff 行号"这类技能明显不足。
建议 500+ step、题目扩到 50+，或直接改用能输出 `search/replace` 格式（无需算行号）
的任务表示 —— 后者可能比堆算力更有效。

**5. 最后才考虑换模型**

本次数据表明瓶颈在 reward 设计与任务表示，而非模型容量。在上述 1-4 项完成前换大模型，
很可能只是让同样的问题跑得更慢。
...
| 6 | 训练后 pass@1 有对比数据 | ✅ | `results/comparison.md`（严格）+ `results/comparison_lenient.md`（高灵敏度 32 采样）。**差异经检验不显著**（Welch t=-0.22），如实报告见 §7.4.3 |
